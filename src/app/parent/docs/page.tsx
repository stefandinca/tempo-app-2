...
export default function ParentDocsPage() {
  ...
  if (error || !client) return <PortalError message={error || "Could not load documents."} />;
  ...
  return (
    <div className="p-4 space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
      <header>
        ...
        <p className="text-neutral-500 text-sm">Downloadable reports and assessments for {client.name}</p>
      </header>
      ...
      <div className="py-20 text-center bg-white dark:bg-neutral-900 rounded-3xl border border-neutral-200 dark:border-neutral-800 shadow-sm">
        ...
        <h3 className="text-lg font-bold text-neutral-900 dark:text-white">No documents shared</h3>
        <p className="text-neutral-500 text-sm mt-1">Shared reports will be listed here for download.</p>
      </div>
    </div>
  );
}