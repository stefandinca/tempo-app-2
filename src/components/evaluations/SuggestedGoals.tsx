...
export default function SuggestedGoals({
  ...
  const resolvedTitle = title ?? t('ev_sum.suggested_iep_goals', { defaultValue: 'Suggested IEP Goals' });
  const resolvedEmptyMessage = emptyMessage ?? t('ev_sum.no_emerging_skills', { defaultValue: 'No emerging skills identified for goal development.' });
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <h4 className="font-bold text-neutral-900 dark:text-white">{resolvedTitle}</h4>
            <p className="text-sm text-neutral-600 dark:text-neutral-400">
              {t('ev_sum.emerging_skills_iep_dev', { defaultValue: '{{count}} emerging skills identified for IEP development', count: goals.length })}
            </p>
          </div>
        </div>
        ...
        <div className="border-t border-amber-200 dark:border-amber-800">
            ...
            >
              {copiedAll ? (
                <>
                  ...
                  {t('ev_sum.copied_all', { defaultValue: 'Copied All!' })}
                </>
              ) : (
                <>
                  <Copy className="w-4 h-4" />
                  {t('ev_sum.copy_all_goals', { defaultValue: 'Copy All Goals' })}
                </>
              )}
            </button>
          </div>
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex-1">
                        ...
                        <div className="bg-neutral-50 dark:bg-neutral-900 rounded p-2">
                          <p className="text-xs font-medium text-neutral-500 mb-1">{t('ev_sum.goal_label', { defaultValue: 'Goal:' })}</p>
                      ...
                      <button
                        ...
                        title={t('ev_sum.copy_goal', { defaultValue: 'Copy goal' })}
                      >
                        {copiedId === goal.id ? (
                          <Check className="w-4 h-4" />
                        ) : (
          ...
          <div className="px-4 py-2 bg-amber-100/50 dark:bg-amber-900/30 border-t border-amber-200 dark:border-amber-800">
            <p className="text-xs text-neutral-500 flex items-center gap-1">
              ...
              {t('ev_sum.goals_autogenerated_note', { defaultValue: 'Goals are auto-generated from emerging skills (scored 0.5 in VB-MAPP, 1 in ABLLS)' })}
            </p>
          </div>
        </div>
      )}
