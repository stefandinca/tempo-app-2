...
import { useCARSEvaluations, useCARSActions } from "@/hooks/useCARS";
...
export default function CARSList({ clientId, clientName }: CARSListProps) {
  ...
  const [viewingSummary, setViewingSummary] = useState<CARSEvaluation | null>(null);
    ...
    <div className="space-y-6">
      ...
      {evaluations.length === 0 ? (
        <div className="bg-white dark:bg-neutral-900 border-2 border-dashed border-neutral-200 dark:border-neutral-800 rounded-3xl p-12 text-center">
          ...
          <h4 className="text-lg font-bold text-neutral-900 dark:text-white mb-1">{t('cars.no_evaluations')}</h4>
          <p className="text-neutral-500 max-w-xs mx-auto mb-6 text-sm">{t('cars.no_evaluations_desc')}</p>
          <button 
            onClick={handleCreateNew}
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div className="space-y-1">
                    <p className="text-[10px] font-bold text-neutral-400 uppercase">{t('cars.total_score')}</p>
                    <p className="text-lg font-black text-indigo-600">
                      {isCompleted ? evaluation.totalScore : '—'}
                    </p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-[10px] font-bold text-neutral-400 uppercase">{t('cars.clinical_classification')}</p>
                    <p className={clsx(
                      "text-[10px] font-bold",
